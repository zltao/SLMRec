...
class SLMRec(BasicModel):
    def __init__(self, 
                 config:dict, 
                 dataset:Dataset):
        super(SLMRec, self).__init__(dataset,config)
        self.__init_weight()
    ...
    def feature_dropout(self,users_idx,items_idx):
        ...
        if self.config["data.input.dataset"] == "kwai":
            ...
            users_sub_1 = self.embedding_user_after_GCN( self.mm_fusion([i_emb_u_sub_1,v_emb_u_sub_1,a_emb_u_sub_1,t_emb_u_sub_1]))
            items_sub_1 = self.embedding_item_after_GCN( self.mm_fusion([i_emb_i_sub_1,v_emb_i_sub_1,a_emb_i_sub_1,t_emb_i_sub_1]))
            users_sub_2 = self.embedding_user_after_GCN( self.mm_fusion([i_emb_u_sub_2,v_emb_u_sub_2,a_emb_u_sub_2,t_emb_u_sub_2]))
            items_sub_2 = self.embedding_item_after_GCN( self.mm_fusion([i_emb_i_sub_2,v_emb_i_sub_2,a_emb_i_sub_2,t_emb_i_sub_2]))
        ...
        labels_user = torch.tensor(list(range(users_sub_2.shape[0]))).to(self.config.device)
        ...
        labels_item = torch.tensor(list(range(items_sub_2.shape[0]))).to(self.config.device)
    ...
    def feature_masking(self,users_idx,items_idx, dropout=False):
        ...
        def compute_graph(u_emb,i_emb,idx):
            all_emb_1 = torch.cat([u_emb, i_emb if rand_idx1 != idx else torch.zeros((self.num_items,self.latent_dim)).to(self.config.device)])
            all_emb_2 = torch.cat([u_emb, i_emb if rand_idx2 != idx else torch.zeros((self.num_items,self.latent_dim)).to(self.config.device)])
    ...
    def fac(self, idx):
        ...
        v_labels = torch.tensor(list(range(x_i_iv.shape[0]))).to(self.config.device)
        ...
        if self.config["data.input.dataset"] != "kwai":
            ...
            a_labels = torch.tensor(list(range(x_iv_iva.shape[0]))).to(self.config.device)
            ...
            t_labels = torch.tensor(list(range(x_iva_ivat.shape[0]))).to(self.config.device)
    ...
    def create_u_embeding_i(self):
        ...
        if self.config["data.input.dataset"] != "kwai":
            ...
            if self.config["data.input.dataset"] == "tiktok":
                ...
                self.t_feat = torch.nn.functional.normalize(self.dataset.t_feat.to(self.config.device).float(),dim=1)
